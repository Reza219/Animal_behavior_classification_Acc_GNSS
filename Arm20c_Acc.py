...
for rs in range(it):
    ...
    for tt in range(len(cn)):
        # training data
        trnx = cf[cf.animal!=cn[tt]][['mx', 'my', 'mz', 'sx', 'sy', 'sz']].values
        trny = cf[cf.animal!=cn[tt]].label.values

        # test data
        tstx = cf[cf.animal==cn[tt]][['mx', 'my', 'mz', 'sx', 'sy', 'sz']].values
        ...
        # train the model
        mdl = MLPClassifier(hidden_layer_sizes=(5,), alpha=1e-4, activation='relu', solver='lbfgs', max_iter=10000)\
            .fit(trnx, trny)
