...
for rs in range(it):
    ...
    for tt in range(len(cn)):
        # training data
        trnxi = cf[cf.animal!=cn[tt]][['mx', 'my', 'mz', 's1x', 's1y', 's1z', 's2x', 's2y', 's2z']].values
        trnxg = cf[cf.animal!=cn[tt]][['speed', 'DtWP', 'error']].values
        trny = cf[cf.animal!=cn[tt]].label.values

        # test data
        tstxi = cf[cf.animal==cn[tt]][['mx', 'my', 'mz', 's1x', 's1y', 's1z', 's2x', 's2y', 's2z']].values
        ...
        # train the model
        mdli = MLPClassifier(hidden_layer_sizes=(7,), alpha=1e-4, activation='relu', solver='lbfgs', max_iter=10000)\
            .fit(trnxi, trny)
