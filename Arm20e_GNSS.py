...
cn = [1, 2, 3, 5, 6, 8]  # animal id

# load data
cf = pd.read_csv('C:/Data/animal activity/Arm20e_Acc_GNSS_features.csv')
...
for rs in range(it):
    ...
    for tt in range(len(cn)):
        # training data
        trnx, trny = cf[cf.animal!=cn[tt]][['speed', 'DtWP', 'error']].values, cf[cf.animal!=cn[tt]].label.values

        # test data
        tstx, tsty = cf[cf.animal==cn[tt]][['speed', 'DtWP', 'error']].values, cf[cf.animal==cn[tt]].label.values
        ...
        # train the model
        mdl = MLPClassifier(hidden_layer_sizes=(4,), alpha=1, activation='relu', solver='lbfgs', max_iter=10000)\
            .fit(trnx, trny)
